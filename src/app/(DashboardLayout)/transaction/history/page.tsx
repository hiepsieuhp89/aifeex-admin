...
function WithdrawalTable() {
  ...
  const handleReject = (id: string) => {
    ...
    if (rejectionReason && adminNote) {
      updateStatus(
        { id, status: "REJECTED", rejectionReason, adminNote },
        {
          onSuccess: () => {
            refetch();
...
function TransactionHistoryPage() {
  ...
  const {
    ...
    type: typeFilter.type ? (typeFilter.type as TransactionType) : undefined,
    start_date: typeFilter.startDate?.format("YYYY-MM-DD"),
    end_date: typeFilter.endDate?.format("YYYY-MM-DD"),
    network: typeFilter.network
      ? (typeFilter.network as TransactionNetwork)
      : undefined,
    user_id:
      typeFilter.user_id || typeFilter.user_id !== ""
        ? (typeFilter.user_id as string)
        : undefined,
  });
      ...
      <Box className="relative flex flex-col items-center justify-center py-8">
        ...
        <Box className="relative flex flex-col items-center gap-2">
          <Box className="p-4 mb-3 rounded-full shadow-lg bg-gradient-to-r from-amber-100 to-orange-100">
            <IconTransactionDollar
              size={36}
              className="text-main-golden-orange"
            />
          </Box>
          <Typography
            variant="h3"
        ...
        <div className="flex flex-col items-start justify-end gap-4 sm:flex-row sm:items-center">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <TextField
              ...
              variant="outlined"
              value={typeFilter.user_id}
              onChange={(e) =>
                setTypeFilter({
                  ...typeFilter,
                  user_id: e.target.value as string,
                })
              }
              className="flex-1 bg-white rounded shadow-sm"
            />{" "}
            <FormControl size="small" style={{ minWidth: 200 }}>
              <DatePicker.RangePicker
                ...
                value={[typeFilter.startDate, typeFilter.endDate]}
                onChange={(dates) => {
                  if (dates) {
                    setTypeFilter({
                      ...typeFilter,
            ...
            <FormControl size="small" style={{ minWidth: 200 }}>
              <InputLabel>Loại mạng</InputLabel>
              <Select
                value={typeFilter.network}
                onChange={(e) =>
                  setTypeFilter({
                    ...typeFilter,
                    network: e.target.value as TransactionNetwork | "",
                  })
                }
          ...
          <>
            <Paper
              ...
              <TableContainer>
                      ...
                      {filteredTransactions.map((transaction: any) => (
                        <TableRow
                          ...
                          <TableCell>
                            <Chip
                              label={
                                getStatusChipProps(transaction.status).label
                              }
                              color={
                                getStatusChipProps(transaction.status).color
                              }
                              size="small"
                              variant="outlined"
                            />
                ...
                {filteredTransactions.length === 0 && (
                  <Paper className="flex flex-col items-center justify-center gap-4 py-8 text-center w-full">
                    <IconMoodSadDizzy size={48} className="text-gray-400" />
                    <Typography
                      fontWeight={400}
                      variant="h6"
                      ...
                      {searchTerm || statusFilter || typeFilter
                        ? "Thử tìm kiếm với điều kiện khác"
