...
import type {
  ...
  IWithdrawResponse,
} from "@/interface/response/transaction";
...
// Nạp tiền
export const rechargeTransaction = async (
  payload: IRechargeRequest
...
export const getAdminWithdrawals = async (params?: any): Promise<any> => {
  const res = await sendGet(
    ConfigTransactionEndPoint.ADMIN_WITHDRAWALS,
    params
  );
  return res;
};
...
export const updateWithdrawalStatus = async (
  ...
  const res = await sendPut(`/withdrawals/${id}/status`, {
    status,
    rejectionReason,
    adminNote,
  });
