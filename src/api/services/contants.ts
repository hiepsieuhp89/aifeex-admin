...
export const ConfigAdminChatEndPoint = {
  SEND_MESSAGE: (userId: string, shopId: string) =>
    `/admin/chat/user/${userId}/shop/${shopId}`,
  GET_MESSAGES: (userId: string, shopId: string) =>
    `/admin/chat/user/${userId}/shop/${shopId}`,
...
export const ConfigFakeReviewEndPoint = {
  ...
  GET_USER_ORDERS: (userId: string) =>
    `/admin/fake-reviews/user-orders?userId=${userId}`,
};
